...
//
// Currently covers the 11 ArbiterLive schools. DigitalSports (26) and the two
// one-off sources are still to come; the output shape is source-agnostic so
// they slot in beside these.
...
(async () => {
    ...
    for (const school of ARBITER_SCHOOLS) {
        try {
            const events = await arbiter.fetchSchool(school, start, end);
            all.push(...events);
    ...
    fs.writeFileSync(OUT, JSON.stringify({
        _comment: 'Generated by scripts/build-schedule.js - do not edit by hand. PARTIAL: currently the 11 ArbiterLive schools only; the 26 DigitalSports schools, Morristown-Beard and Delbarton are not in here yet, so this is not a complete conference schedule. Games only - practices are filtered out. Sorted by date then start time, de-duplicated so a fixture listed by both schools appears once. Cancelled games are kept and carry status.',
        generated: new Date().toISOString(),
        range: { start, end },
        sources: report,
        coverage: { schoolsFetched: report.length, schoolsInConference: 39, complete: false },
