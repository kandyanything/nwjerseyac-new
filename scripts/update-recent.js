...
//
// Fetches the same three sources build-schedule.js does, scoped to a short
// window, then splices the result into the existing data/schedule.json in
...
(async () => {
    ...
    // three days still returns its whole current month, and the iCal feeds
    // return their entire calendar regardless of what is asked for. Filtering
    // down to the actual window here is what keeps this "hourly, next few
    // days" rather than "hourly, whatever each source felt like sending back."
    ...
    //
    // splitSports is only persisted from the point this script was added
    // onward, so schedule.json may not carry it yet. Fall back to reading the
    // split back off the existing games themselves - anything already named
    // "Boys X" / "Girls X" / "Coed X" is split, whether or not the field
    // exists - rather than silently un-splitting every sport in the window
    // until the next nightly rebuild catches up.
